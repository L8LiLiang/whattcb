...
#define NODEVIEW_MARGIN_BETWEEN_BUTTON_LABEL 2
...
@property (strong, nonatomic) NSMutableArray *arrayLabels; //no use now
@property (strong, nonatomic) NSMutableDictionary *indexToIconDict;// no use now
...
- (instancetype)initWithArray:(NSArray *)array  type:(NodeType)type status:(NodeStatus)status nodes:(NSArray *)nodes
{
    if (self = [super init]) {
        _contents = array;
        _nodes = nodes;
        self.arrayLabels = [NSMutableArray new];
        self.font = [UIFont systemFontOfSize:16];
        self.indexToIconDict = [NSMutableDictionary new];
        self.arrayForLabelToLayout = [NSMutableArray new];
        self.type = type;
        self.status = status;
        [self addBackgroundAndButton];
        [self makeContents];
    }
    return self;
}
...
-(void)setContents:(NSArray *)contents
{
    ...
    self.tmpViewForMakeConstraint = nil;
    for (UILabel *label in self.arrayForLabelToLayout) {
        [label removeFromSuperview];
    }
    [self.arrayForLabelToLayout removeAllObjects];
    [self makeContents];
...
{
    _status = status;
    [self changeBackgroundAndButtonByType];
}
...
- (void)changeBackgroundAndButtonByType
{
    UIImage *imageBackgroundForView;
    UIImage *resizeImage;
    
    if (_type == NODE_TYPE_AUTO) {
        ...
        self.backgroundImageView.image = resizeImage;
        
         UIImage *imageBackForBtn = [UIImage imageNamed:@"signIn_telephone"];
        [self.btnContact setBackgroundImage:imageBackForBtn forState:UIControlStateNormal];
    }else {
        imageBackgroundForView = [UIImage imageNamed:@"pop_green"];
        resizeImage = [imageBackgroundForView resizableImageWithCapInsets:UIEdgeInsetsMake(45, 40, 16, 16)];
        self.backgroundImageView.image = resizeImage;
        UIImage *imageBackForBtn = [UIImage imageNamed:@"signIn_signIn"];
        if (self.status == NODE_STATUS_FINISHED) {
            imageBackForBtn = [UIImage imageNamed:@"signIn_hasSigned"];
        }
        [self.btnContact setBackgroundImage:imageBackForBtn forState:UIControlStateNormal];
    }
...
- (void)addBackgroundAndButton
{
    UIImage *imageBackgroundForView;
    UIImage *resizeImage;
    if (_type == NODE_TYPE_MANUAL) {
        imageBackgroundForView = [UIImage imageNamed:@"pop_green"];
        resizeImage = [imageBackgroundForView resizableImageWithCapInsets:UIEdgeInsetsMake(45, 40, 16, 16)];
    }else {
        imageBackgroundForView = [UIImage imageNamed:@"pop_gray"];
...
- (void)makeContents
{
    for (int i = 0; i < _contents.count; i++) {
        NSDictionary *dictContentItem = _contents[i];
        ...
        [label2 makeConstraints:^(MASConstraintMaker *make) {
            make.left.equalTo(label1.right).offset(NODEVIEW_MARGIN_BETWEEN_LABEL1_LABEL2);
            ...
            make.right.lessThanOrEqualTo(self.btnContact.left).offset(-NODEVIEW_MARGIN_BETWEEN_BUTTON_LABEL);
            make.height.greaterThanOrEqualTo(@15);
        }];
...
- (void)btnContactClicked:(UIButton *)sender
{
    if (self.type == NODE_TYPE_MANUAL) {
        if (self.status != NODE_STATUS_FINISHED) {
            if ([self.delegate respondsToSelector:@selector(flowNodeViewSign:success:)]) {
                [self.delegate flowNodeViewSign:self success:^{
                    self.status = NODE_STATUS_FINISHED;
                }];
            }
        }
    }else {
        if ([self.delegate respondsToSelector:@selector(flowNodeViewCall:)]) {
            [self.delegate flowNodeViewCall:self];
        }
    }
}
