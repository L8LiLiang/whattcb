...
@implementation FeeConfirmListCell
...
+ (instancetype)cellWithTableView:(UITableView *)tableView {
    static NSString *CellIdentifier = @"FeeConfirmListCell";
    FeeConfirmListCell *cell = [tableView dequeueReusableCellWithIdentifier:CellIdentifier];
    if (cell == nil) {
        cell = [[FeeConfirmListCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:CellIdentifier];
    }
    return cell;
}
...
+ (CGFloat)cellHeightWithFeeConfirmItemArray:(NSArray *)feeConfirmItemArray {
    return (feeConfirmItemArray.count == 1) ? 195 : 320;
}
...
- (void)tapNextView:(UITapGestureRecognizer *)tap {
    if ([self.feeConfirmListCellDelegate respondsToSelector:@selector(nextViewOnTap:)]) {
        [self.feeConfirmListCellDelegate nextViewOnTap:tap];
    }
}

- (void)feeConfirmButtonClicked:(UIButton *)sender {
    if ([self.feeConfirmListCellDelegate respondsToSelector:@selector(feeConfirm:)]) {
        [self.feeConfirmListCellDelegate feeConfirm:sender];
    }
}
